...
public final class PCanvas extends Canvas implements MouseListener, MouseMotionListener {
    ...
    public void updateComputations() {
        ...
        Ps = new Vector[ALL.length];
        int k = ALL.length;
        // zeichne edges
        for (int i = 0; i < ALL.length; ++i) {
          // System.out.println("CANVAS ALL i:" + i + ", x: " + ALL[i].getX() + ", y: " + ALL[i].getY() );
          Ps[i] = new Vector(ALL[i].getX(), ALL[i].getY());
        }
        // GG = Vector.toGeneralPath(Ps);
        ...
        for (int i = 0; i < Ps.length-5; ++i) { 
            Vector exitpoint = new Vector(Ps[k-i-2]);
            Vector l1 = new Vector(Vector.cross(Ps[k-i-1], exitpoint)); 
            Vector l2 = new Vector(Vector.cross(exitpoint, Ps[k-i-3]));
            Vector l3 = new Vector(Vector.cross(exitpoint, Ps[k-i-4]));
            Vector l4 = new Vector(Vector.cross(exitpoint, Ps[k-i-5]));
            Vector m = new Vector(Vector.cross(Ps[k-i-4], Ps[k-i-5]));
            crUp[i] = model.CrossRatio(m, l1, l2, l3, l4); }
        // compute cross ratio downwards
        double[] crDown = new double[Ps.length-5];
        for (int i = 0; i < Ps.length-5; ++i) { 
            Vector exitpoint = new Vector(Ps[i+2]);
            Vector l1 = new Vector(Vector.cross(Ps[i+1], exitpoint)); 
            Vector l2 = new Vector(Vector.cross(exitpoint, Ps[i+3]));
            Vector l3 = new Vector(Vector.cross(exitpoint, Ps[i+4]));
            Vector l4 = new Vector(Vector.cross(exitpoint, Ps[i+5]));
            Vector m = new Vector(Vector.cross(Ps[i+4], Ps[i+5]));
