...
/* MMatrix.java
 ...
 MMatrix is a *lightweight* set of linear 
 ...
 Note that "MMatrix" represents an arbitrary array 
...
public class MMatrix {
  ...
  public MMatrix(int r, int c, double fill) {
  ...
  public MMatrix(int r, int c, char code) {
...
//  }	
  ...
  public MMatrix transpose() {
    ...
    MMatrix t = new MMatrix(columns, rows);
    int i, j;
    for (i = 0; i<rows; i++) {
      for (j = 0; j<columns; j++) {
  ...
  public MMatrix diag() {
    ...
    */
    MMatrix t = new MMatrix(rows, 1);
    if(columns==1){
      t = new MMatrix(rows, rows);
    }
    int i, k, j;
    for (i = 0; i<rows; i++) {
      if(columns>1){
  ...
  public static MMatrix add(MMatrix m1, MMatrix m2){
    // Return the matrix m = m1 + m2
    MMatrix m=new MMatrix(m1.rows,m1.columns);
  ...
  public static MMatrix subtract(MMatrix m1, MMatrix m2){
    // Return the matrix m = m1 - m2
    MMatrix m=new MMatrix(m1.rows,m1.columns);
    if ((m1.rows == m2.rows)&&(m1.columns==m2.columns)) {
      int i,j;
  ...
  public static MMatrix multiply(double d, MMatrix m1){
  ...
  public static MMatrix multiply(MMatrix m1, MMatrix m2){
    /* MMatrix-MMatrix or MMatrix-vector product
       returns m=m1*m2
       m1 can be a 1x1 MMatrix for scalar-MMatrix product
    */
    MMatrix m = new MMatrix(0);
    if (m1.columns == m2.rows) {
      // matrix product
      double sum = 0;
      int k = 0;
      m = new MMatrix(m1.rows,m2.columns);
      int i,j;
      for (i=0; i<m.rows; i++) {
	for (k=0; k<m2.columns; k++) {
      ...
      m = new MMatrix(m2.rows,m2.columns);
      int i,j;
      for (i=0; i<m.rows; i++) {
	for (j=0; j<m.columns; j++) {
  ...
  public static MMatrix divide(MMatrix m1, MMatrix m2) {
    ...
    */
    ...
    if ((m2.columns == 1)&&(m2.rows == 1)) {
      ...
      m=new MMatrix(m1.rows,m1.columns);
      int i,j;
      for (i=0; i<m.rows; i++) {
	for (j=0; j<m.columns; j++) {
	 ...
	 system Ax=b via QR, where A=m2, b=m1, and x is returned. */
      m=new MMatrix(m2.rows,1);
      MMatrix Q=m2.Q();
      MMatrix R=m2.R();
      MMatrix b=multiply(Q.transpose(),m1);
      int i,j;
      double sum = 0;
  ...
  public MMatrix sub(int r1, int r2, int c1, int c2) {
    // returns the submatrix (r1:r2,c1:c2) (Moeler notation)
    // requires r2>=r1, c2>=c1
    MMatrix A = new MMatrix(r2 - r1 + 1, c2 - c1 + 1);
    int i, j;
    for (i = r1; i<=r2; i++) {
      for (j = c1; j<=c2; j++) {
  ...
  public MMatrix appendCols(MMatrix x){
    // append the column vectors in x to this matrix
    MMatrix M=new MMatrix(rows,columns+x.columns);
    int i,j;
    for(i=0;i<rows;i++){
      for(j=0;j<columns;j++){
  ...
  public MMatrix appendRows(MMatrix x){
    // append the row vectors in x to this matrix
    MMatrix M=new MMatrix(rows+x.rows,columns);
    int i,j;
    for(i=0;i<columns;i++){
      for(j=0;j<rows;j++){
  ...
  public MMatrix flipud(){
    // flip this matrix vertically
    MMatrix M=new MMatrix(rows,columns);
    int k,j;
    for(k=0;k<rows;k++){
      for(j=0;j<columns;j++){
  ...
  public MMatrix permute(int a1, int a2, char c) {
  ...
  public double norm() {
    /* returns the Frobenius norm (MMatrix), or Euclidean norm (Vector)
       This is the default norm for a MMatrix object. Use the Norm
  ...
  public MMatrix  Q() {
    ...
    */
    MMatrix P = new MMatrix(rows, rows, 'I');
    MMatrix A = new MMatrix(this);
        MMatrix AA;
        MMatrix PP;
    int i, j;
    MMatrix v;
...
//    MMatrix P = new MMatrix(rows, columns, 'I');
//    MMatrix I = new MMatrix(rows, columns, 'I');
//    MMatrix A = new MMatrix(this);
//    int i, j, k;
//    MMatrix v;
//    
//    for(j = 0; j<columns-2; j++) {
//      v = new MMatrix(rows, 1);
//      v.element[j][0] = 1;
...
//    MMatrix L = new MMatrix(rows, columns, 'I');	// the lower triangle
//    MMatrix U = this;	// this matrix to be transformed to upper triangular
//    MMatrix G = new MMatrix(rows, columns, 'I');	
...
//	For example, if A is a MMatrix of  positive real numbers, then
//	A.leig(0.01) returns the largest eigenvalue to at least two
//	digits of accuracy.
//    */
//    Vector qr;
//    MMatrix Q = new MMatrix(rows, columns);
//    MMatrix R = new MMatrix(rows, columns);
//    MMatrix A = new MMatrix(this);	// initialized
//    int i = 1;
...
//      qr = A.qr();
//      Q = (MMatrix)qr.elementAt(1);
//      R = (MMatrix)qr.elementAt(0);
//      A = multiply(R,Q);
//      i++;
...
//  }
  ...
  public MMatrix  sort() {
    /* Sort this object in the following way:
       1. If this is a column or row vector, sort the vector
          and return a two column MMatrix containing the sorted
	  vector in the first column, and the permutation indices
	  in the 2nd column.
       2. If this is a rectangular MMatrix, sort the rows according
          to the entries in the first column.
    */
    MMatrix R = new MMatrix();
    int i,k;
  ...
  public MMatrix  order() {
    /*	Sorts this vector, returning a ranked vector,
	(here, 'vector' indicates a 1-d MMatrix, not the Java class Vector).
	'order' is used so as to not confuse with the usual definition of rank
    */
    MMatrix S = this.sort();
    MMatrix y = new MMatrix(S.rows, 1);
  ...
  // The following methods are used internally by the MMatrix class:
